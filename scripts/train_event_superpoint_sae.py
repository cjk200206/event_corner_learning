...
if __name__ == '__main__':
    ...
    for iter in range(flags.num_epochs):
            ...
            for event_vox, label_vox, heatmap, sae_50, sae_75, sae_100 in tqdm.tqdm(validation_loader): 
                ...
                with torch.no_grad():
                    ...
                    try:
                        ...
                        loss_d,_,_,_ = descriptor_loss(desc,desc_transform,inv_homographies,mask_valid=mask_valid,device=desc.device,lamda_d=10000)
            ...
            for label_vox,sae_50 in tqdm.tqdm(validation_loader): 
                ...
                with torch.no_grad():
                    ...
                    try:
                        ...
                        loss_a, accuracy_a = compute_superpoint_loss(semi, label_3d)
                        loss_b, accuracy_b = compute_superpoint_loss(semi_transform, label_3d_transform)
                        inv_homographies = inv_homography.expand(desc.shape[0],-1,-1)
                        mask_valid = torch.ones_like(desc[:,0]).unsqueeze(1)
                        loss_d,_,_,_ = descriptor_loss(desc,desc_transform,inv_homographies,mask_valid=mask_valid,device=desc.device,lamda_d=10000)
        ...
        if len(validation_loader) != 0:
            ...
            validation_loss_d = sum_loss_d.item()/len(validation_loader)
        else:
            validation_loss = min_validation_loss
            validation_loss_d = 0
            validation_accuracy = 0
        print(f"Validation Loss {validation_loss:.4f}  Accuracy {validation_accuracy:.4f} Validation Loss_d {validation_loss_d:.4f}")
        ...
        writer.add_scalar("validation/loss_d", validation_loss_d, iteration)
        ...
        if len(training_loader) != 0:
            ...
            training_loss_d = sum_loss_d.item() / len(training_loader)
            training_accuracy = sum_accuracy / len(training_loader)
        else:
            training_loss = min_validation_loss
            training_loss_d = 0
            training_accuracy = 0
        print(f"Training Iteration {iteration:5d}  Loss {training_loss:.4f}  Accuracy {training_accuracy:.4f} Loss_d {training_loss_d:.4f}")

        writer.add_scalar("training/accuracy", training_accuracy, iteration)
        writer.add_scalar("training/loss", training_loss, iteration)
        writer.add_scalar("training/loss_d", training_loss_d, iteration)
